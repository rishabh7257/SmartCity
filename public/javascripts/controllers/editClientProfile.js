'use strict';
wfms.controller("EditClientProfileCtrl", function($scope, $http, $modalInstance, isEdit, $rootScope, DataService, $window) {
    $scope.states = [{
        name: 'ALABAMA',
        abbreviation: 'AL'
    }, {
    ...
    $scope.okay = function() {
        if (!(isValidPostalCode($scope.ZipCode))) {
            ...
            if ($scope.firstName && $scope.lastName && $scope.email && $scope.phonenumber && $scope.address && $scope.city && $scope.ZipCode) {
                var paramsPerson = {
                ...
                DataService.putData("/api/editPerson", paramsPerson).success(function(response) {
                    ...
                    //.success(
                    DataService.putData('api/updateClient', params).success(function(response) {
